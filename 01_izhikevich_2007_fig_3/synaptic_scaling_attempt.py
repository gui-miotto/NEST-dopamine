...
class Network():
    def __init__(self, ext_thr_ratio=.87, master_seed=42, A_plus=.2, A_minus=1.25):
        """[summary]
        Model parameters
        ...
        self.DA_pars = {
            ...
            'Wmax' : self.J['E'] * 5., # Maximal synaptic weight  
        ...
        self.trial_duration = 20000. #10000.0  # Trial duration
        self.cue_duration = 3. # Cue stimulus duration
        self.cue_stim = 300. # Stimulus delivered to cue neurons in pF
    ...
    def simulate(self, n_trials=400, plot=False, save_plot_dir=None):
        # (warning: nest.ResetNetwork wont reboot synaptic weights, 
        # we would have to do it manually or just rebuild the network)
        self._build_network()
        ...
        for trial in range(1, n_trials+1):
            ...
            plastic_ws, total_plastic_ws = self._synaptic_rescaling(events['E'])
            ...
            if plot:
                self._plot(trial, A_minus_B, events, n_events, plastic_ws, \
    ...
    def _simulate_one_trial(self):
        ...
        if A_minus_B > 0:
            wait_time = self.max_DA_wait_time - (decision_spikes['A'] - decision_spikes['B']) * 100.
    ...
    def _synaptic_rescaling(self, events_E):
        delta_t = self.trial_duration - self.eval_time_window - self.max_DA_wait_time
        valid_time = nest.GetKernelStatus()['time'] - delta_t
        delta_t_sec = delta_t / 1000.
        

        for node_e in self.nodes['E']:
            ind = np.where(events_E['senders']==node_e)[0]
            times = events_E['times'][ind]
            n_events = len(np.where(times > valid_time)[0])

            # If the trial is long enough, we should never enter the if. But just in case,
            # in order to avoid a division by zero, we set n_events = 1.            
            if n_events == 0:  
                n_events = 1
                print('WARNING! n_events == 0 while rescaling')
            
            fr = n_events / delta_t_sec
            gain = 1. / fr
            learning_rate = .5

            cnx = nest.GetConnections(self.nodes['E'], (node_e,))
            weights = np.array(nest.GetStatus(cnx, 'weight'))
            #new weights:
            weights += learning_rate * (gain - 1.) * weights
            nest.SetStatus(cnx, params='weight', val=weights)


        all_plastic_syn=nest.GetConnections(self.nodes['E'], self.nodes['E'])
        curr_plastic_w = nest.GetStatus(all_plastic_syn, 'weight')
        curr_total_plastic_w = np.sum(curr_plastic_w)
        return curr_plastic_w, curr_total_plastic_w
    ...
    def _plot(self, trial_num, A_minus_B, events, n_events, curr_plastic_w,\
        ...
        # raster plot
        plt.subplot(3,3,(1,2))
        plt.title('Current trial raster plot')
        prev_max_snd = 0
        for pop in ['S', 'A', 'B', 'exc', 'inh', 'DA']:
            if n_events[pop] == 0:
                continue
            ...
            shifted_snds = snds - np.min(snds) + prev_max_snd + 1
            s = 16. if pop == 'DA' else 5.
            mkr = '*' if pop == 'DA' else '.'
            plt.scatter(times, shifted_snds, marker=mkr, s=s, label=pop)
            prev_max_snd = np.max(shifted_snds)
