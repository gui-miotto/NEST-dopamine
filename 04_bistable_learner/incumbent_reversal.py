...
if __name__ == '__main__':
    ...
    success_history = exp.train_brain(n_trials=150, aversion=True, save_dir=args.outdir)
    result = np.sum(success_history[-100:])
    # Run reversal learning
    success_history = exp.train_brain(n_trials=150, aversion=True, rev_learn=True, save_dir=args.outdir)
    result += np.sum(success_history[-100:])
